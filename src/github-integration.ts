...
export class PRComplianceReview {
  ...
  private formatComment(report: ComplianceReport): string {
    ...
    comment += `---\n`;
    comment += `*Generated by [Curtis Compliance](https://curtis.ai) at ${new Date(report.timestamp).toLocaleString()}*`;
  ...
  private async updateStatusCheck(report: ComplianceReport, shouldBlock: boolean): Promise<void> {
    ...
    await this.octokit.repos.createCommitStatus({
      ...
      target_url: `https://curtis.ai/compliance/${this.context.owner}/${this.context.repo}/pr/${this.context.prNumber}`
    });
  }
